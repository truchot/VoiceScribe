...
final class SileroVAD {
    ...
    private func processInternal(samples: [Float], timestamp: TimeInterval) {
        ...
        while frameBuffer.count >= config.frameSamples {
            let frame = Array(frameBuffer.prefix(config.frameSamples))
            frameBuffer.removeFirst(config.frameSamples)
            processFrame(frame)
...
struct RingBuffer<T> {
    ...
    func toArray() -> [T] {
        ...
        // Return in correct order: from writeIndex to end, then start to writeIndex

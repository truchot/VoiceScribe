...
final class SystemAudioCapture: NSObject, ObservableObject {
    ...
    private let bufferQueue = DispatchQueue(label: "com.voicescribe.system-audio-buffer")
    ...
    private func updateAvailableApps(from content: SCShareableContent) {
        ...
        print("🔍 Detected \(detected.count) video apps: \(detected.map(\.name).joined(separator: ", "))")
    }
    ...
    func startCapturing(app: DetectedApp? = nil, chunkDuration: TimeInterval = 3.0) async throws {
        ...
        print("🔊 System audio capture started for: \(target.name)")
    }

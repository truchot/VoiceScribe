...
struct EmotionalState: Codable {
    ...
    var label: EmotionLabel {
        ...
        if valence > 0.3 && arousal > 0.2 { return .enthusiastic }
        if valence > 0.3 && arousal < -0.2 { return .satisfied }
        if valence < -0.3 && arousal > 0.2 { return .frustrated }
        if valence < -0.3 && arousal < -0.2 { return .disengaged }
        if dominance < -0.3 { return .hesitant }
        if dominance > 0.3 && valence > 0 { return .confident }
        if arousal > 0.4 { return .animated }
        return .neutral
    }
}
...
enum EmotionLabel: String, Codable, CaseIterable {
    ...
    var color: Color {
        switch self {
        case .enthusiastic: return .green
        case .satisfied: return .mint
        case .confident: return .blue
        case .animated: return .cyan

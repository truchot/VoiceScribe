...
final class WhisperTranscriber {
    ...
    private let transcribeQueue = DispatchQueue(label: "com.voicescribe.whisper", qos: .userInitiated)
    ...
    func loadModel() throws {
        ...
        print("🧠 Loading Whisper model: \(config.modelPath)")
        ...
        print("✅ Whisper model loaded in \(String(format: "%.1f", elapsed))s")
        isLoaded = true
    }
    ...
    func transcribe(
        ...
        completion: @escaping (Result<[TranscriptionSegment], WhisperError>) -> Void
    ) {
        ...
        transcribeQueue.async { [config] in
            ...
            lang.withCString { langPtr in
                ...
                params.speed_up = config.speedUp
                ...
                print("📝 Whisper: \(numSegments) segments in \(String(format: "%.2f", inferenceTime))s (RTF: \(String(format: "%.2f", rtf))x)")
    ...
    func transcribeSync(samples: [Float], chunkTime: TimeInterval) throws -> [TranscriptionSegment] {
        var result: Result<[TranscriptionSegment], WhisperError>?
